...
esp_err_t bsp_sdcard_mount(void)
{
    ...
    return esp_vfs_fat_sdmmc_mount(CONFIG_BSP_SD_MOUNT_POINT, &host, &slot_config, &mount_config, &bsp_sdcard);
}

esp_err_t bsp_sdcard_unmount(void)
{
    return esp_vfs_fat_sdcard_unmount(CONFIG_BSP_SD_MOUNT_POINT, bsp_sdcard);
}
...
void bsp_leds_init(void)
{
    const gpio_config_t led_io_config = {
        ...
        .mode = GPIO_MODE_OUTPUT_OD, // GPIO3 is connected directly to the LED, so we use Open-drain here
