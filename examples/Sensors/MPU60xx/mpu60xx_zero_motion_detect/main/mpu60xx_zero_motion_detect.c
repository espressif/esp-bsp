...
void read_data(void *mpu_handle)
{
    ...
    while (1) {
        ...
        if (xSemaphoreTake(s_bin_sem, 20 / portTICK_PERIOD_MS) == pdPASS) {
            ...
            if (count % 2) { // zero motion is raised both when zero motion begins and ends
                printf("motion_detector: ZERO MOTION DETECTED\n");
            }
        }
    }
}
...
void app_main(void)
{
    ...
    mpu60xx_event_detect_config_t z_mot_det = {
        .event_threshold = 50,
        .event_duration = 10,
    ...
    mpu60xx_intrpt_config_t md_intr_conf =  {
        ...
        .interrupt_latch = MPU60XX_INTERRUPT_LATCH_50US,
    ...
    mpu60xx_en_EventDetection(dev_6050_handle, &z_mot_det, MPU60XX_EVENT_ZERO_MOTION);
