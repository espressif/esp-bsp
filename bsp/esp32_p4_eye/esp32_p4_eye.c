...
static const char *TAG = "esp32_p4_eye";
...
esp_err_t bsp_feature_enable(bsp_feature_t feature, bool enable)
{
    ...
    switch (feature) {
    case BSP_FEATURE_SD:
        ret |= rtc_gpio_init(BSP_SD_EN);
        ret |= rtc_gpio_set_direction(BSP_SD_EN, RTC_GPIO_MODE_OUTPUT_ONLY);
        ret |= rtc_gpio_pulldown_dis(BSP_SD_EN);
        ret |= rtc_gpio_pullup_dis(BSP_SD_EN);
        ret |= rtc_gpio_hold_dis(BSP_SD_EN);
        ret |= rtc_gpio_set_level(BSP_SD_EN, enable);
        ret |= rtc_gpio_hold_en(BSP_SD_EN);
        break;
