...
#define BSP_I2S_GPIO_CFG                                                                     \
    {                                                                                        \
        .bclk = BSP_I2S_SCLK, .ws = BSP_I2S_LCLK, .dout = BSP_I2S_DOUT, .din = BSP_I2S_DSIN, \
        .invert_flags = {                                                                    \
            .mclk_inv = false,                                                               \
            .bclk_inv = false,                                                               \
            .ws_inv   = true,                                                                \
...
esp_err_t bsp_audio_init(const i2s_std_config_t *i2s_config)
{
    ...
    if (i2s_tx_chan && i2s_rx_chan) {
        ESP_LOGW(TAG, "Audio was initialized before");
        return ESP_OK;
    }
    ...
    if (i2s_tx_chan != NULL) {
        ESP_GOTO_ON_ERROR(i2s_channel_init_std_mode(i2s_tx_chan, p_i2s_cfg), err, TAG,
                          "I2S TX channel initialization failed");
    ...
    if (i2s_rx_chan != NULL) {
        ESP_GOTO_ON_ERROR(i2s_channel_init_std_mode(i2s_rx_chan, p_i2s_cfg), err, TAG,
                          "I2S RX channel initialization failed");
    ...
    ESP_LOGI(TAG, "I2S config: sample_rate=%lu, data_bit_width=%d, slot_mode=%ld",
             (unsigned long)p_i2s_cfg->clk_cfg.sample_rate_hz, p_i2s_cfg->slot_cfg.data_bit_width,
