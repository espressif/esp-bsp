...
esp_err_t esp_gpio_append_expander_handler(esp_io_expander_handle_t handler, uint32_t start_io_num)
{
    ...
    ioexp_range_node_t *node = (ioexp_range_node_t *)heap_caps_malloc(sizeof(ioexp_range_node_t), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (node == NULL) {
...
err:
    portEXIT_CRITICAL(&s_ioexp_lock);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Either requested IO expander range overlaps with existing range or the same handler is already appended");
        free(node);
    }
    return err;
}

esp_err_t esp_gpio_remove_expander_handler(esp_io_expander_handle_t handler)
{
    if (handler == NULL) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t __wrap_gpio_set_pull_mode(gpio_num_t gpio_num, gpio_pull_mode_t pull)
{
    ...
    if (!handler->write_pullup_en_reg || (pud == IO_EXPANDER_PULL_DOWN && !handler->write_pullup_sel_reg)) {
        pud_valid = false;
    }
    if (!pud_valid) {
