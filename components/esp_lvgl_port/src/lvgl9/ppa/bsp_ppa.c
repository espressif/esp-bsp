...
size_t get_cache_line_size(const void *addr)
{
...
#define WIDTH             (1920)
#define HEIGHT            (1536)
#define SRC_PX_SIZE       (3)
#define DEST_PX_SIZE      (3)
...
void dma2d_ppa_speed_test(void)
{
    int src_bytes = SRC_PX_SIZE;
    int dest_bytes = DEST_PX_SIZE;
    int dest_w = WIDTH;
    int dest_h = HEIGHT;
    int count = 100;
    asm_dsc_t dsc = {
        .dest_w = dest_w,
        .dest_h = dest_h,
        .dest_stride = dest_w * dest_bytes,
        .opa = 0xFF,
        .src_stride = dest_w * src_bytes
    };
    dsc.dest_buf = heap_caps_malloc(dsc.dest_stride * dsc.dest_h, MALLOC_CAP_SPIRAM | MALLOC_CAP_CACHE_ALIGNED);
    dsc.src_buf = heap_caps_malloc(dsc.src_stride * dsc.dest_h, MALLOC_CAP_SPIRAM | MALLOC_CAP_CACHE_ALIGNED);
    for (int i = 0; i < count; i++) {
        esp_dma2d_convert_for_lvgl(&dsc, dest_bytes, src_bytes);
    }
    for (int i = 0; i < count; i++) {
        esp_ppa_fill_for_lvgl(&dsc, dest_bytes);
    }
    for (int i = 0; i < count; i++) {
        esp_ppa_blend_area_for_lvgl(&dsc, dest_bytes, src_bytes);
    }
    for (int i = 0; i < count; i++) {
        esp_ppa_fill_opa_for_lvgl(&dsc, dest_bytes);
    }
...
static lv_result_t find_start_addr_and_offset(const uint8_t *src_buf, uint8_t **pic_addr, uint32_t *offsetx, uint32_t *offsety)
{
    ...
    if (src_buf >= buf1 && src_buf < buf1 + pic_size) {
        ...
        addr = buf2;
    } else if ((uint32_t)src_buf == ALIGN_UP((uint32_t)src_buf, cache_line_size)) {
        *pic_addr = src_buf;
        *offsetx = 0;
        *offsety = 0;
        return LV_RESULT_OK;
...
static lv_result_t _check_arg(uint32_t dest_w, uint32_t dest_h, const uint8_t *mask_buf)
{
    if (lv_display_get_color_format(NULL) == LV_COLOR_FORMAT_XRGB8888) {
        return LV_RESULT_INVALID;
    }
...
extern esp_err_t dma2d_color_convert(color_space_pixel_format_t src_format, color_space_pixel_format_t dest_format,
...
int esp_dma2d_convert_for_lvgl(asm_dsc_t * dsc, const uint8_t dest_px_size, uint32_t src_px_size)
{
    ...
    color_space_pixel_format_t src_format = {
        .color_space = COLOR_SPACE_RGB,
        .pixel_format = src_px_size == 2 ? COLOR_PIXEL_RGB565 : COLOR_PIXEL_RGB888,
    };
    color_space_pixel_format_t dest_format = {
        .color_space = COLOR_SPACE_RGB,
        .pixel_format = dest_px_size == 2 ? COLOR_PIXEL_RGB565 : COLOR_PIXEL_RGB888,
    };

    uint8_t *dest_pic_buf = NULL;
    uint32_t dest_offx = 0;
    uint32_t dest_offy = 0;
    if (find_start_addr_and_offset(dsc->dest_buf, &dest_pic_buf, &dest_offx, &dest_offy) == LV_RESULT_INVALID) {
        return LV_RESULT_INVALID;
    }

    if (dma2d_color_convert(src_format, dest_format, dsc->dest_w, dsc->dest_h,
                        dsc->src_buf, 0, 0, dsc->src_stride / src_px_size, dsc->dest_h,
                        dest_pic_buf, dest_offx, dest_offy, pic_w, pic_h) == ESP_FAIL) {
        return LV_RESULT_INVALID;
    }
...
int esp_ppa_fill_opa_for_lvgl(asm_dsc_t *dsc, uint32_t dest_px_size)
{
    ...
    // In fact we need free, but frequent malloc and free memory takes up a lot of time
    // So we just malloc once, and do not free
    // The max memory consumption is about 1000KB
    static uint8_t *fg_pic = NULL;
    ...
    if (fg_max_size < out_buf_size_align) {
        ...
        fg_pic = heap_caps_aligned_calloc(cache_line_size, fg_max_size, sizeof(uint8_t), MALLOC_CAP_SPIRAM);
    }
    ...
    if (err != ESP_OK) {
        printf("ppa register client err: %d\n", err);
        goto _err;
    }
    ...
    if (count++ % 21 == 0) {
        ESP_LOGI(TAG, "%s, %d ms, (x:%ld, y:%ld)[w:%ld, h:%ld][spx:%d, dpx:%ld], %.1f MB/s", 
...
int esp_ppa_blend_area_for_lvgl(asm_dsc_t * dsc, uint32_t dest_px_size, uint8_t src_px_size)
{
    ...
    uint16_t cache_line_size = CONFIG_CACHE_L2_CACHE_LINE_SIZE;
...
int esp_ppa_fill_for_lvgl(asm_dsc_t *dsc, uint32_t dest_px_size)
{
    ...
    static ppa_client_handle_t ppa_client_handle = NULL;

    uint32_t out_buf_size_align = ALIGN_UP(stride * pic_h, CONFIG_CACHE_L2_CACHE_LINE_SIZE);
    if (ppa_client_handle == NULL) {
        ppa_client_config_t ppa_client_config = {
    ...
    esp_err_t err = ppa_do_fill(ppa_client_handle, &fill_trans_config);
    int duration = esp_timer_get_time() - start;
    uint32_t data_size = dsc->dest_w * dsc->dest_h * dest_px_size;
    float speed = data_size * 0.95367 / duration;
    static uint32_t count = 0;
    if (count++ % 21 == 0) {
        ESP_LOGI(TAG, "%s, %d ms, (x:%ld, y:%ld)[w:%ld, h:%ld][spx:%d, dpx:%ld], %ld KB, %.1f MB/s",
