...
bool LV_ATTRIBUTE_FAST_MEM lv_draw_esp_dma2d_task_supported(const lv_draw_task_t *t, bool *is_convert)
{
    ...
    if (!image_task_basic_supported(dsc)) {
        return false;
    }
    ...
    if (!lv_draw_esp_dma2d_cf_info(src_cf, &src_info)) {
        return false;
    }
    if (!lv_draw_esp_dma2d_cf_info(dst_cf, &dst_info)) {
        return false;
    }
    ...
    if (convert) {
        ...
        if (!lv_draw_esp_dma2d_tx_csc_option(src_cf, dst_cf, &csc_option)) {
            return false;
        }
    }
...
static int32_t LV_ATTRIBUTE_FAST_MEM dma2d_evaluate(lv_draw_unit_t *draw_unit, lv_draw_task_t *t)
{
    ...
    if (!lv_draw_esp_dma2d_task_supported(t, &is_convert)) {
        return 0;
    }
...
static int32_t LV_ATTRIBUTE_FAST_MEM dma2d_dispatch(lv_draw_unit_t *draw_unit, lv_layer_t *layer)
{
    ...
    if (t == NULL || t->preferred_draw_unit_id != DRAW_UNIT_ID_ESP_DMA2D) {
        return LV_DRAW_UNIT_IDLE;
    }
