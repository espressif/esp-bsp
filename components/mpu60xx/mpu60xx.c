 ...
 */
static esp_err_t register_isr(mpu60xx_intrpt_config_t *interrupt_configuration);
...
esp_err_t mpu60xx_Interrupt_pin_configuration (mpu60xx_handle_t mpu_handle, mpu60xx_intrpt_config_t *interrupt_configuration)
{
    ...
    cmd[1] = int_pin_cfg;
    ESP_RETURN_ON_ERROR (mpu60xx_write_register(mpu_handle, MPU60xx_ADDR, cmd, 2), s_TAG, "unable to configure MPU60xx interrupt pin behaviour");
...
esp_err_t mpu60xx_en_EventDetection(mpu60xx_handle_t mpu_handle, mpu60xx_event_detect_config_t *mdc, mpu60xx_event_t event)
{
    ...
    switch ( event ) {
    case MPU60XX_EVENT_FREE_FALL:
        mot_thr_reg = MPU60xx_FF_THR;
        mot_dur_reg = MPU60xx_FF_DUR;
        break;
    case MPU60XX_EVENT_MOTION_DETECT:
        mot_thr_reg = MPU60xx_MOT_THR;
        mot_dur_reg = MPU60xx_MOT_DUR;
        break;
    case MPU60XX_EVENT_ZERO_MOTION:
        mot_thr_reg = MPU60xx_ZMOT_THR;
        mot_dur_reg = MPU60xx_ZMOT_DUR;
        break;
    }

    cmd[0] = mot_thr_reg;
    cmd[1] = mdc->event_threshold;
    ESP_RETURN_ON_ERROR (mpu60xx_write_register(mpu_handle, MPU60xx_ADDR, cmd, 2), s_TAG, "unable to write to mot_threshold");


    cmd[0] = mot_dur_reg;
    cmd[1] = mdc->event_duration;
    ESP_RETURN_ON_ERROR (mpu60xx_write_register(mpu_handle, MPU60xx_ADDR, cmd, 2), s_TAG, "unable to write to mot_duration");
...
esp_err_t mpu60xx_enEventDetectInterrupt(mpu60xx_handle_t mpu_handle, bool active, mpu60xx_event_t event)
{
    ...
    ESP_RETURN_ON_ERROR ( mpu60xx_write_register(mpu_handle, MPU60xx_ADDR, cmd, 2), s_TAG, "unable to write to MPU60xx_INT_EN_REG register");
...
{
    ESP_RETURN_ON_ERROR (i2c_master_probe(mpu_handle->bus_handle, MPU60xx_ADDR, 50), s_TAG, "mpu60xx not detected on i2c");
    ...
    cmd[1] = 0x80;
    ESP_RETURN_ON_ERROR (mpu60xx_write_register(mpu_handle, MPU60xx_ADDR, cmd, 2), "", "" );
...
mpu60xx_handle_t mpu60xx_create ( i2c_master_bus_handle_t bus_handle, uint8_t mpu60xxaddress)
{
    mpu60xx_handle_t my_mpu60xxhandle = malloc(sizeof(mpu60xx_dev_t));

    if (my_mpu60xxhandle == NULL) {
        ESP_LOGE(s_TAG, "unable to allocate memory to initialize mpu60xx handle");
        return my_mpu60xxhandle;
    }

    i2c_device_config_t mpu60xxconfig = {
        .dev_addr_length = I2C_ADDR_BIT_LEN_7,
        .device_address = mpu60xxaddress,
        .scl_speed_hz = CONFIG_MPU60xx_I2C_CLK_SPEED,
    };
    ESP_LOGI(s_TAG, "adding mpu60xx as device to bus\n");
    i2c_master_bus_add_device(bus_handle, &mpu60xxconfig, &(my_mpu60xxhandle->i2c_dev_handle) );
    ESP_LOGI(s_TAG, "device added to bus\n");

    my_mpu60xxhandle->bus_handle = bus_handle;

    return my_mpu60xxhandle;
}


void mpu60xx_remove ( mpu60xx_handle_t *mpu60xxptr)
{
    i2c_master_bus_rm_device( (*mpu60xxptr)->i2c_dev_handle);
    free(*mpu60xxptr);
    *mpu60xxptr = NULL; // now MPU60xx handle is not a dangling pointer
}
